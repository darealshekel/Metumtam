(function(root){
 ...
 function defaults(c){return {levels:Object.fromEntries(c.cores.map(x=>[x.id,x.initial])),goals:Object.fromEntries(c.cores.map(x=>[x.id,x.max])),daily:100,weekly:0,weeklyDungeon:'none',weeklyQuest:false,stock:0,erdaStock:0,extraLevels:{generalCore1:0}};}
 function clean(c,p={}){if(!p||typeof p!=='object'||Array.isArray(p))p={};const d=defaults(c);for(const core of c.cores){d.levels[core.id]=clamp(p.levels?.[core.id]??core.initial,core.initial,core.max);d.goals[core.id]=clamp(p.goals?.[core.id]??core.max,d.levels[core.id],core.max);}d.extraLevels.generalCore1=clamp(p.extraLevels?.generalCore1??0,0,30);d.erdaStock=clamp(p.erdaStock??0,0,20);d.weeklyDungeon=Object.hasOwn(dungeonRewards,p.weeklyDungeon)?p.weeklyDungeon:'none';d.weeklyQuest=p.weeklyQuest===true;for(const key of ['daily','weekly','stock'])d[key]=clamp(p[key]??d[key],0,10000000);return d;}
