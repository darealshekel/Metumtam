(function(root){
 'use strict';
 const S=root.HexaStats||(typeof require==='function'?require('./hexa-stats.js'):null);
 const clamp=(v,min,max)=>Math.max(min,Math.min(max,Number.isFinite(Number(v))?Math.floor(Number(v)):min));
 const dungeonRewards={none:0,high_mountain:40,angler_company:55,nightmare_paradise:70};
 function defaults(c){return {hexaStats:S.defaults(),levels:Object.fromEntries(c.cores.map(x=>[x.id,x.initial])),goals:Object.fromEntries(c.cores.map(x=>[x.id,x.max])),daily:100,weekly:0,weeklyDungeon:'none',weeklyQuest:false,stock:0,erdaStock:0,extraLevels:{generalCore1:0}};}
 function clean(c,p={}){if(!p||typeof p!=='object'||Array.isArray(p))p={};const d=defaults(c);d.hexaStats=S.clean(p.hexaStats);for(const core of c.cores){d.levels[core.id]=clamp(p.levels?.[core.id]??core.initial,core.initial,core.max);d.goals[core.id]=clamp(p.goals?.[core.id]??core.max,d.levels[core.id],core.max);}d.extraLevels.generalCore1=clamp(p.extraLevels?.generalCore1??0,0,30);d.erdaStock=clamp(p.erdaStock??0,0,20);d.weeklyDungeon=Object.hasOwn(dungeonRewards,p.weeklyDungeon)?p.weeklyDungeon:'none';d.weeklyQuest=p.weeklyQuest===true;for(const key of ['daily','weekly','stock'])d[key]=clamp(p[key]??d[key],0,10000000);return d;}
 function enhancement(c,p,id){const core=c.cores.find(x=>x.id===id),level=p.levels[id];if(!core||core.type==='HEXA Stat')return {available:false,reason:'Upgrade costs are not available for this node.'};if(level>=core.max)return {available:false,maxed:true,reason:'This node is at maximum level.'};if(!c.costs?.[id])return {available:false,reason:'Upgrade costs are not available for this node.'};const price=cost(c,id,level,level+1);return {available:true,from:level,to:level+1,...price,affordable:p.stock>=price.f&&p.erdaStock>=price.e};}
 function enhance(c,p,id){const state=enhancement(c,p,id);if(!state.available||!state.affordable)return null;const updated=clean(c,p);updated.levels[id]=state.to;updated.goals[id]=Math.max(updated.goals[id],state.to);updated.stock-=state.f;updated.erdaStock-=state.e;return updated;}
 ...
 function settings(value={}){return {bossMode:value?.bossMode==='kaling'?'kaling':'general',orderMode:value?.orderMode==='erda'?'erda':'fragments',includeThirdSkill:value?.includeThirdSkill===true};}
 ...
 function sourceOrder(c,options){const s=settings(options),order=(s.bossMode==='kaling'?c.kalingOrders?.[s.orderMode]:null)||c.orders?.[s.orderMode]||(s.orderMode==='fragments'?c.order:[]);return order.filter(step=>s.includeThirdSkill||step.id!=='skillCore3');}
