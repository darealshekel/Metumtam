...
Hover a skill for a compact MapleStory Wiki preview. It never intercepts pointer input and disappears immediately when the pointer leaves the skill. The HEXA Order strip shares its skill colors with View full order. Enhance Node replaces the separate Next upgrade panel: it follows the first remaining order milestone but displays only the next level, its description, and the materials for that single enhancement. Each Enhance advances one skill level even with no owned materials. HEXA Stat nodes remain completion markers using the source's milestone cost; they are not modeled as skill levels. Optional material balances decrease to a minimum of zero; Undo restores the level and both balances. Sol Erda ownership is saved per class (0–20), and fragments share the existing forecast inventory. Older exports default Sol Erda ownership to zero. In preview mode, the button returns to saved progression without changing levels or spending materials. Sol Janus is tracked manually outside the damage order.
...
HEXA upgrades opens a dedicated class library at #classes. My classes shows saved class plans and their fragment-goal progress; All classes offers all 54 supported classes, with wiki artwork, name search, and category filters. Each planner has a shareable #class=<id> link; saved levels remain local to the device. Browser Back returns to the library. HEXA progress appears above HEXA Order. Terms of Service and Privacy are linked in the footer. The About the data and Export plan buttons have been removed.

Single-enhancement regression checks: run `node tests/engine.test.cjs`. The suite walks all 54 classes in both resource modes with the third skill core enabled and disabled, checking one-level upgrades, exact aggregate costs, stat completion, custom goals, and completion.

Saved-class cards include a touch-sized Remove button with confirmation. Removing a class deletes only its local plan; opening a class without editing does not add it to My classes. Number editing preserves the focused input and its ancestors in the DOM so typing does not repeatedly blur/refocus the field. Invalid saved entries are skipped while valid class plans are retained. If browser storage is blocked, editing continues for the current visit and a visible notice explains the persistence limit.

Browser feedback regression checks (with a local server on port 4183 and an open Playwright CLI session): `playwright-cli run-code --filename tests/browser-feedback.js`. This checks number entry, focus and caret stability, saved-class removal, invalid saved entries, blocked storage/external requests, and a touch viewport. It uses isolated browser contexts.
